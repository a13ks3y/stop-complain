...
// Let's let and globalize testing tools so
// there's no need to require them in each test
let sinon = require('sinon');
let chai = require('chai');
let expect = require('chai').expect;
let chaiAsPromised = require('chai-as-promised');
let chaiJsonScheme = require('chai-json-schema');
 ...
 */
chokidar.watch( '*.spec.js', { persistent: true } )
    .on( 'add', path => fileList.push( path ) )
.on( 'change', path => runSuite() )
.on( 'ready', () => runSuite() );

chokidar.watch( ['*.js', '*.jsx'], { persistent: true } )
    .on( 'change', path => runSuite() );